...
kube_config = "~/.kube/config" #Change per requirements
k = logging.getLogger()
...
def create_kube_ml():
    ...
    # create namespace
    namespace_cmd = f"kubectl create namespace mskubesample"
    logging.debug(subprocess.check_output(namespace_cmd, shell=True))
    ...
    try:
        # deploy kube-ml
        kube_ml_path = os.path.join(os.getcwd(), "k8s", "kube-ml", "examples", "kubernetes")
        os.chdir(kube_ml_path)
        kube_ml_deploy_cmd = f"kubectl create -f kube-ml-clusterip-linkerd.yaml -n kube-ml"
        logging.debug(subprocess.check_output(kube_ml_deploy_cmd, shell=True))
        logging.info("Deployed kube-ml.")
        os.chdir(os.getcwd())
        
        # wait until deployment is ready
        time.sleep(int(os.getenv("SLEEP_TIME")))
        while not check_kube_ml_health():
            time.sleep(10)
        logging.info("kube-ml is alive.")
        
    except Exception as err:
        logging.error(f"Error while deploying kube-ml: {err}")
...
def deploy(config_file_path):
    ...
    time.sleep(10)
    utils.create_from_yaml(kube_client, config_file_path, True, os.getenv("namespace"))


def kube_update_deployment_from_file(yaml_file, cpu_limit, memory_limit, number_of_replicas):
    ...
    # Use the Kubernetes API to update the deployment
...
def kube_create_deployment_from_image(name, port, docker_path):
    os.system(f"kubectl create namespace {os.getenv('namespace')}")
...
def check_kube_ml_health():
    """
    Checks if kube-ml is healthy by hitting its healthcheck URL.
    Returns True if the healthcheck succeeds, False otherwise.
    """
    kube_ml_host = os.getenv("base_url")
    kube_ml_port = os.getenv("kube_port")
    healthcheck_url = f"https://{kube_ml_host}:{kube_ml_port}/healthcheck"
