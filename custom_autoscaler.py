...
import preprocess
import train_models
from kube_utils import k8s_update_deployment, set_prometheus_info
...
def scale() -> None:
    ...
    if check_target_status(target_status):
        ...
        best_parameters = train_models.get_best_parameters_hpa(cpu_limit=parameter_status[0], memory_limit=parameter_status[1],
...
def improve() -> None:
    ...
    loadtesting.get_prometheus_data(date, 0, int(os.getenv("PERIOD_HH")), int(os.getenv("PERIOD_MM")))
    # filter data
    preprocess.filter_data(date)
    curr = preprocess.get_filtered_data(date)
    prev = preprocess.get_filtered_data(os.getenv("LAST_TRAINED_DATA"))
    # combine old and new data
    preprocess.combine_data([prev, curr], date)
    # train all models
    train_models.train_for_all_targets(os.getenv("ALGORITHM"))
...
def scale_k_hpa() -> None:
    ...
    """
    # current status
    parameter_status, target_status = loadtesting.get_status("webui")
    # average usage
